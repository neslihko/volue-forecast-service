...
### Detailed Component Diagram (Matching Platform Architecture)
...
graph TB
    ...
    subgraph RepoLayer["Repository Layer"]
        direction LR
        FWR[ForecastWrite<br/>Repository]
        FRR[ForecastRead<br/>Repository]
        PRR[PositionRead<br/>Repository]
        PPR[PowerPlant<br/>Repository]
        CDC["ForecastDbContext<br/>(EF Core)"]
    end
    
    subgraph DataLayer["Database"]
        PG[("PostgreSQL<br/><br/>Tables: companies,<br/>power_plants, forecasts")]
    end
    
    subgraph MessagingLayer["Messaging"]
        EP[RabbitMqEventPublisher /<br/>NullEventPublisher]
        RMQ["RabbitMQ Exchange<br/>forecast.events"]
        CONS["Consumers:<br/>Trading System,<br/>Analytics, Dashboard"]
    end
    ...
    MW --> FS
    MW --> PS[PositionService]
    MW --> PPS[PowerPlantService]
    ...
    style Clients fill:#FFF8E1,stroke:#F57C00,stroke-width:2px
    style APILayer fill:#E3F2FD,stroke:#1976D2,stroke-width:2px
    style ServiceLayer fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px
    style RepoLayer fill:#E8F5E9,stroke:#388E3C,stroke-width:2px
    style DataLayer fill:#E1F5FE,stroke:#0277BD,stroke-width:3px
    style MessagingLayer fill:#FFF9C4,stroke:#F57F17,stroke-width:2px
```
